...
public class MonkeyEntity extends AnimalEntity {
	public static final TagKey<ConfiguredStructureFeature<?, ?>> MONKEY_LOCATED = TagKey.of(Registry.CONFIGURED_STRUCTURE_FEATURE_KEY, Utils.NewIdentifier("monkey_located"));
	...
	static class LeadToNearbyTreasureGoal extends Goal {
		...
		public boolean shouldContinue() {
			...
			return !(new BlockPos(blockPos.getX(), this.monkey.getY(), blockPos.getZ())).isWithinDistance(this.monkey.getPos(), 4.0D) && !this.noPathToStructure;
		}
		...
		public void start() {
			if (this.monkey.world instanceof ServerWorld serverWorld) {
				...
				BlockPos blockPos2 = serverWorld.locateStructure(MONKEY_LOCATED, blockPos, 50, false);
				if (blockPos2 != null) {
		...
		public void stop() {
			BlockPos blockPos = this.monkey.getTreasurePos();
			if ((new BlockPos(blockPos.getX(), this.monkey.getY(), blockPos.getZ())).isWithinDistance(this.monkey.getPos(), 4.0D) || this.noPathToStructure) {
		...
		public void tick() {
			...
			if (this.monkey.isNearTarget() || this.monkey.getNavigation().isIdle()) {
				...
				Vec3d vec3d2 = NoPenaltyTargeting.findTo(this.monkey, 16, 1, vec3d, 0.39269909262657166);
				if (vec3d2 == null) {
					vec3d2 = NoPenaltyTargeting.findTo(this.monkey, 8, 4, vec3d, 1.5707963705062866);
				}
				...
				if (vec3d2 != null) {
					...
					if (!world.getFluidState(blockPos).isIn(FluidTags.WATER) || !world.getBlockState(blockPos).canPathfindThrough(world, blockPos, NavigationType.WATER)) {
						vec3d2 = NoPenaltyTargeting.findTo(this.monkey, 8, 5, vec3d, 1.5707963705062866);
					}
				}
				...
				this.monkey.getLookControl().lookAt(vec3d2.x, vec3d2.y, vec3d2.z, (float) (this.monkey.getMaxHeadRotation() + 20), (float) this.monkey.getMaxLookPitchChange());
				this.monkey.getNavigation().startMovingTo(vec3d2.x, vec3d2.y, vec3d2.z, 1.3);
