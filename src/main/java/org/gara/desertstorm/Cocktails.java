...
public final class Cocktails {
	...
	public static final Potion SUNSET = registerPotion("sunset", new StatusEffectInstance(StatusEffects.ABSORPTION, 3600), new StatusEffectInstance(StatusEffects.REGENERATION, 1800));
	public static final Potion ICED = registerPotion("iced", new StatusEffectInstance(FREEZING, 170));
	public static final Potion HOT_COCOA = registerPotion("hot_cocoa", new StatusEffectInstance(StatusEffects.SPEED, 600), new StatusEffectInstance(StatusEffects.HASTE, 600));
	...
	public static final Potion MULTIVITAMIN = registerPotion("multivitamin", new StatusEffectInstance(StatusEffects.NIGHT_VISION, 3600), new StatusEffectInstance(StatusEffects.LUCK, 3600), new StatusEffectInstance(StatusEffects.INVISIBILITY, 600));
	...
	private static class MidasTouch extends CustomStatusEffect {
		public static final List<Block> GOLD_BLOCKS = List.of(Blocks.GOLD_BLOCK, Blocks.RAW_GOLD_BLOCK, Blocks.GOLD_ORE, Blocks.NETHER_GOLD_ORE, Blocks.DEEPSLATE_GOLD_ORE);
		...
		public void applyUpdateEffect(LivingEntity entity, int amplifier) {
			if (entity instanceof ServerPlayerEntity serverPlayerEntity && !entity.isSpectator()) {
				...
				if (!(state.hasBlockEntity() || state.getBlock().getBlastResistance() > 30)) {
					Block goldBlock = GOLD_BLOCKS.get(entity.getRandom().nextInt(GOLD_BLOCKS.size()));
					serverWorld.setBlockState(down, goldBlock.getDefaultState());
				}
			}
		}
	}
	...
	private static class Random extends InstantStatusEffect {
		...
		public void applyInstantEffect(@Nullable Entity source, @Nullable Entity attacker, LivingEntity target, int amplifier, double proximity) {
			if (!target.isSpectator()) {
				var random = Registry.STATUS_EFFECT.getRandom(target.getRandom());
				if (random.isPresent()) {
					var statusEffect = random.get().value();
					if (statusEffect.isInstant()) {
						statusEffect.applyInstantEffect(source, attacker, target, amplifier, proximity);
					} else {
						target.addStatusEffect(new StatusEffectInstance(statusEffect, 30 * 20, amplifier), source);
					}
				}
