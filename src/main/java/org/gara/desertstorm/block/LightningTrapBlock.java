...
public class LightningTrapBlock extends Block implements BlockEntityProvider {
	...
	public ActionResult onUse(BlockState blockState, World level, BlockPos blockPos, PlayerEntity player, Hand interactionHand, BlockHitResult blockHitResult) {
		if (!blockState.get(CHARGED) && (player.isHolding(DesertStorm.BATTERY_ITEM) || player.isHolding(Items.TRIDENT) && EnchantmentHelper.hasChanneling(player.getStackInHand(interactionHand)))) {
	...
	public void onSteppedOn(World world, BlockPos blockPos, BlockState blockState, Entity entity) {
		if (blockState.get(CHARGED)) {
			...
			if (entity instanceof PlayerEntity player && !Utils.IsSurvival(player)) {
				return;
			}
